...
describe("Restaurant Search", () => {
  ...
  it("Successfully displays restaurants", () => {
    ...
    cy.get(".sc-AxmLO > :nth-child(1)").click();

    cy.get('[data-testid="Location SearchTextInput"]').should(
      "have.value",
      "Atlanta, Georgia"
    );
    ...
    cy.get("[data-testid=cuisineList] > :nth-child(1)").click();

    cy.get("[data-testid=cuisineList] > :nth-child(1)").click();

    cy.get('[data-testid="Next PageButton"]').click();

    cy.get(".sc-AxjAm").should("have.text", "Recommended Restaurants");

    cy.get(".sc-fzplWN > :nth-child(1)").should("be.visible");
  });
});
