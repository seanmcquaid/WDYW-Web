...
const CuisineListPage: React.FC = () => {
  ...
  useEffect(() => {
    if (isMounted.current) {
      ...
      axios
        .get(
          `https://developers.zomato.com/api/v2.1/cuisines?city_id=${entity_id}`,
          config
        )
        .then(({ data }) => {
  ...
  const cuisineOnClickHandler = useCallback(
    (cuisine: Cuisine) => {
      const leftoverCuisineList = cuisineList.filter(
        (item) => item !== cuisine
      );
  ...
  return (
    <PageContainer>
      <Header>
        <H1>
          {errorMessage.length > 0 ? errorMessage : `Cuisine List For ${title}`}
        </H1>
      </Header>
      <MainContent>
        <CuisineList
        ...
        />
        <Button title="Next Page" onClick={nextPageOnClickHandler} />
      </MainContent>
    </PageContainer>
  );
};
